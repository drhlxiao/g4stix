...
DetectorMessenger::DetectorMessenger(DetectorConstruction *theDet)
  ...
  fDetectorSelectionCmd->SetGuidance(
      "Only construct a single detector if 0<= det < 32 else all detectors");
...
void DetectorMessenger::SetNewValue(G4UIcommand *command, G4String newValue) {
  if (command == fSetAttenStatusCmd) {
    fDetector->SetAttenuatorStatus(
        fSetAttenStatusCmd->GetNewBoolValue(newValue));
