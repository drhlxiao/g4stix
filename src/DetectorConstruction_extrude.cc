...
G4LogicalVolume *DetectorConstruction::ConstructCdTeDetector()
{
	...
	G4double anodeThickness=(15+15+100)*1e-6*mm; //130 nm
	G4double cathodeThickness=15* 1e-6*mm; //130 nm
										  //thickness negligible  compared to the thickness  uncertainty


	G4Box *detModuleWorld= new G4Box("CdTeModuleWorld", 11.0 * mm *0.5 ,
			12 * mm *0.5, (14.4 +1 ) * mm*0.5 + anodeThickness/2 + cathodeThickness/2);

	G4Box *detModuleBaseOuter= new G4Box("CdTeModuleBaseOuter", 11.0 * mm *0.5 ,
			12 * mm *0.5, (14.4) * mm*0.5);

	G4Box *detModuleBaseInner= new G4Box("CdTeModuleBase", 10.0 * mm *0.5 ,
			10 * mm *0.5, 14 * mm*0.5);

										  //
	G4Box *CdTeBox = new G4Box("CdTeDet", 5 * mm, 5 * mm, 0.5 * mm + cathodeThickness/2. +  anodeThickness/2.);
	...
	G4Box *smallPixelGeo =
		new G4Box("smallPixel", smallW / 2, smallH / 2, cdteThickness * mm);

	G4LogicalVolume *detModuleLog =
		new G4LogicalVolume(detModuleWorld, Vacuum, "detModuleWorld", 0, 0, 0);
	G4LogicalVolume *detModuleOuterLog =
		new G4LogicalVolume(detModuleBaseOuter, Copper, "detModuleBaseOuter", 0, 0, 0);
	//caliste base material, is unknown
	G4LogicalVolume *detModuleInnerLog=
		new G4LogicalVolume(detModuleBaseInner, Vacuum, "detModuleBaseInner", 0, 0, 0);
	...
	G4double bigPixelColor[] = {0.5, 0.6, 0.7, 0.5};
	...
	G4double smallPixelColor[] = {0.2, 0.2, 0.5, 0.5};
	...
	G4double detZ= (cathodeThickness-anodeThickness)/2. ;
...
//				copyNb, checkOverlaps);
	...
	new G4PVPlacement(0, G4ThreeVector(0, 0, 0.5*mm+anodeThickness/2 ), 
			CdTeCathodeLog, "cathode", XrayDetLog, false,0, true);
	new G4PVPlacement(0, G4ThreeVector(0, 0, -0.5*mm-cathodeThickness/2 ), 
			CdTeAnodeLog, "anode", XrayDetLog, false,0, true);
	//place the cathode and anode in the detector


	new G4PVPlacement(0, G4ThreeVector(0,0,0), detModuleInnerLog, "detModuleInner", detModuleOuterLog, 
			false, 	0, checkOverlaps);
	new G4PVPlacement(0, G4ThreeVector(0,0,  -(cdteThickness+cathodeThickness+anodeThickness)/2), detModuleOuterLog, "detModuleOuter", detModuleLog,
			false, 		0, checkOverlaps);

	G4double detBaseColor[] = {0.3, 0.6, 0.5, 0.9};
	SetVisAttrib(detModuleOuterLog, detBaseColor[0], detBaseColor[1], detBaseColor[2], detBaseColor[3]);

	//14.4+1  + anode, cathode, 
	G4double cdTeZ= (14.4*mm+cdteThickness + anodeThickness +cathodeThickness)/2 -  (0.5 * mm + cathodeThickness/2. +  anodeThickness/2. );

	new G4PVPlacement(0, G4ThreeVector(0,0.8*mm,cdTeZ), XrayDetLog, "XrayDetLog", detModuleLog, 
			false, 	0, checkOverlaps);
...
G4VPhysicalVolume *DetectorConstruction::Construct() {
	...
	{
		G4cout<<"CAD models will be not imported"<<G4endl;
