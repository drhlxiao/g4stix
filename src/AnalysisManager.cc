...
void AnalysisManager::ProcessStep(const G4Step *aStep) {
  ...
  if (killTracksEnteringGrids) {
    if (eventID % 10000 == 0) {
      G4cout << "WARNING: Tracks are killed in grids! "
             << "Don't kill them if you are not running simulations for the "
                "grid effects!!"
             << G4endl;
    }

    if (volName == "gridStrip") {
...
*/
  ...
  if (numPhysTreeFilled < MAX_NUM_TREE_TO_FILL) {
    if (volName.contains("att_")) { // don't store too many tracks
      const G4TrackVector *secondary = aStep->GetSecondary();
      for (size_t lp = 0; lp < (*secondary).size(); lp++) {
        processType = (*secondary)[lp]->GetCreatorProcess()->GetProcessType();
        processSubtype =
            (*secondary)[lp]->GetCreatorProcess()->GetProcessSubType();
        physTree->Fill();
        numPhysTreeFilled++;
      }
    }
  ...
  // end of physics processes tracking
  ...
  // G4cout<<volName<<" "<<preVolName<<" "<<nextVolName<<G4endl;
  // volName is the preVol, it is always the same
  //

  // kill the tracks in CdTe, this line has to be removed
  ...
  detectorID = postStep->GetTouchable()->GetCopyNumber(3);
  pixelID = postStep->GetTouchable()->GetCopyNumber(0);
